      ...
      <vuestic-widget class="info-widget brand-info">
        <div class="info-widget-inner">
          <div class="stats">
            ...
            <div v-if="loadedData" class="stats-number">
              <i class="ion ion-arrow-up-c text-primary stats-icon"></i> {{uzoraka_danas}}
            </div>
      ...
      <vuestic-widget class="info-widget">
        <div class="info-widget-inner">
          <div class="stats">
            ...
            <div v-if="loadedData" class="stats-number">
              <i class="ion ion-arrow-up-c text-primary stats-icon"></i> {{kompletirano_uzoraka}}
            </div>
        ...
        <div class="info-widget-inner">
          <div class="info-widget-inner has-chart">
            <div class="stats">
              ...
              <div v-if="loadedData" class="stats-number">
                {{kompletirano_testova}}
              </div>
              <div class="stats-title">{{'Kompletirano testova'}}</div>
            </div>
            <div class="chart-container">
              <vuestic-progress-bar type="circle" ref="circleProgress" 
                :colorName="'white'" :backgroundColorName="'danger'" :startColorName="'danger'"></vuestic-progress-bar>
            </div>
          </div>
        </div>
      </vuestic-widget>
      ...
      <vuestic-widget class="info-widget brand-info">
        <div class="info-widget-inner">
          <div class="stats">
            ...
            <div v-if="loadedPatients" class="stats-number">
              <i class="ion ion-md-people stats-icon icon-wide"></i> {{pacijenata}}
            </div>
...
<script>
  ...
  export default {
    ...
    mounted() {      
      http.get('/dashboard/info' + '?token=' + this.$store.state.token + '&site=' + this.$store.state.site, {
        ...
        .then(res => {
          if(res.data.json.empty != undefined){
            ...
            var perCompleted = 0 
            var perCompleted = (res.data.json.realizovanoTestova / res.data.json.ukupnoTestova)
            this.$refs.circleProgress.$data.value = Math.round(perCompleted * 100)    
      ...
      http.get('dashboard/patients/' + '?token=' + this.$store.state.token + '&site=' + this.$store.state.site, {
