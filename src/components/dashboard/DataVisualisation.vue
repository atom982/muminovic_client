      ...
      <div class="row">
        ...
        <div class="col-md-8 visina">
          <div v-if="loadedChart " class="chart-widget visina">
            <vuestic-chart v-bind:data="podatakChart" :options="VerticalBarChartOptions" type="vertical-bar"></vuestic-chart>
          </div>
          <div v-if="!loadedChart" class="chart-widget visina">            
            <h6 style='text-align: center; line-height: 380px;'>
              <Spinner name="ball-beat" color="#e34a4a" />
  ...
  export default {
    ...
    methods: {
      pripremiPodatkeDonutChart: function(callback) {
        http.get('/dashboard/donut' + '?token=' + this.$store.state.token + '&site=' + this.$store.state.site, {
            ...
            if (res.data) {
              if (res.data.json.empty) { 
                ...
                var podatakSample = {
                  ...
                  datasets: [{
                    label: 'Broj uzoraka',
                    backgroundColor: [palette.danger, palette.warning, palette.info, palette.primary],
                    data: [res.data.json.zaprimljenUzorak, res.data.json.uObradiUzorak, res.data.json.realizovanUzorak, res.data.json.obradjenUzorak]
                  }]
                }
                this.loadedSample = true
                this.DonutChartOptions = {
      ...
      pripremiPodatkeVerticalBarChart: function(callback) {
        http.get('/dashboard/bar' + '?token=' + this.$store.state.token + '&site=' + this.$store.state.site, {
