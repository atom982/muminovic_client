    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <vuestic-widget v-if="main" :headerText="header">
          <div class="row">            
            <div class="col-md-3">
              <p style="padding-left: 10px;">Pacijent:<strong> {{ pacijent }}</strong>
                <span>&nbsp<i v-bind:class="{'fa fa-check fa-lg success-icon icon-right input-icon' : odobren}"></i></span>
              </p>
              <hr>
            </div>
            ...
            <div class="col-md-3 btn-container">
              <button v-if="!verificiran" class="btn btn-info btn-sm wizard-back" @click.prevent="odobriRezultate(rezultati)"><span class="fa fa-check-square-o"></span>{{ ' ODOBRITE'}}</button>
              <button v-if="verificiran" class="btn btn-primary btn-sm wizard-back" @click.prevent="ukloniOdobrenje(rezultati)"><span class="fa fa-undo"></span>{{ ' ODOBREN '}}</button>
          </div>
          </div>
              ...
              <tbody>
                <tr align="middle" v-for="test in rezultati" v-bind:class="{
                  ...
                  <td v-if="!test.multidata">
                    ...
                    <select v-if="test.select_options != undefined" style="width:110px" v-show="test.edit == true" v-model="test.rezultat">
                      <option v-for="option in test.select_options">
                        {{ option }}
                      </option>
                    </select>
                    <input v-if="test.select_options === undefined" v-show="test.edit == true" v-model="test.rezultat" style="width:110px">
                  </td>
                  <td v-if="test.multidata && test.realizovan">
                    <button :id="test.labassay_id" title='Pregled rezultata' class="btn btn-primary btn-micro sq" @click.prevent="prikazAnalita($event, test.ime, test.analit, test.rezultat, id, test.odobren)"><span :id="test.labassay_id" @click.prevent="prikazAnalita($event, test.ime, test.analit, test.rezultat, id, test.odobren)" class="glyphicon glyphicon-search"></span> VIŠE</button>
                  </td>
                  <td v-if="test.multidata && !test.realizovan">
                  ...
                  <td v-if="!verificiran && calculated && test.calculated">
                     <button @click.prevent="calculate($event, $route.params.id, test.labassay_id, test.ime, test.id)" title='Izračunaj rezultat'
                      :id='test.test' class='btn btn-primary btn-micro'><span :id='test.test' class='fa fa-calculator' ></span></button>
                  </td>
                  <!-- PONOVI -->
                  <td v-if="!test.multidata && !verificiran && !test.manual && !test.calculated">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="btn-container">
                ...
                <button v-if="!odobren" class="btn btn-primary btn-sm wizard-back pull-right" @click.prevent="sacuvajRezultate(rezultati)"><span class="fa fa-save"></span>{{ ' SAČUVAJ'}}</button>
              </div>
            </div>
          </div>
        </vuestic-widget>
  ...
  export default {
    ...
    mounted() {
      bus.$on('sacuvajAnalite', () => {
        http.post('/rezultati/sacuvaj/analite/' + this.$route.params.id, {
            analiti: this.analiti,
    ...
    created() {
      ...
      http.get('rezultati/show/' + this.$route.params.id + '?token=' + this.$store.state.token + '&site=' + this.$store.state.site, {})
        .then(res => {
          ...
          this.header = 'Odobravanje rezultata za uzorak: ' + this.$route.params.id
    ...
    methods: {      
      calculate: function(event, ide, id, ime, edited) { 
        http.post('/rezultati/calculate/' + this.$route.params.id, { // Upis nove vrijednosti rezultata
            _id: id,
            token: this.$store.state.token,
            site: this.$store.state.site,
            ime: ime,
      ...
      sacuvajRezultate: function(rezultati) {
        ...
        http.post('/rezultati/sacuvaj/' + this.$route.params.id, {
      ...
      ukloniOdobrenje: function(rezultati) { 
        http.post('/rezultati/ukloni/odobrenje/sve/' + this.$route.params.id, {
            testovi: rezultati,
            site: this.$store.state.site,
            token: this.$store.state.token
          })
