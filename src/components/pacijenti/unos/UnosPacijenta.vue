<template>
  <div class="form-elements">
    <div class="row">
      <div class="col-md-12">
        <vuestic-widget :headerText="'pacijenti.unos.UnosPacijenta.trnslr001' | translate">
          <!-- Unos pacijenata -->
          <form>
            <div class="row">
              ...
              <div class="col-md-6">
                <fieldset>
                  ...
                  <!-- Konzumira duhan -->
                  <vuestic-simple-select
                    :label="'pacijenti.unos.UnosPacijenta.trnslr006' | translate"
                    v-model="duhan"
                    name="duhan"
                    title=" "
                    ref="duhanSelect"
                  ...
                  <!-- Dijabetes -->
                  <vuestic-simple-select
                    :label="'pacijenti.unos.UnosPacijenta.trnslr007' | translate"
                    v-model="dijabetes"
                    name="dijabetes"
                    title=" "
                    ref="dijabetesSelect"
                  ...
                  <button class="btn btn-primary" @click.prevent="dodajPacijenta">
                    ...
                    &nbsp{{'pacijenti.unos.UnosPacijenta.trnslr010' | translate}}
                    <!-- Sačuvaj -->
                  </button>
                </fieldset>
              </div>
...
export default {
  ...
  methods: {
    ...
    validatePhone: function(telefon) {
      if (
        /^\d{9}$/.test(String(telefon)) ||
        /^\d{10}$/.test(String(telefon)) ||
        /^[0]{2}[3]{1}[8]{1}[7]{1}\d{8}$/.test(String(telefon)) ||
        /^[0]{2}[3]{1}[8]{1}[7]{1}\d{9}$/.test(String(telefon)) ||
        /^[+]{1}[3]{1}[8]{1}[7]{1}\d{8}$/.test(String(telefon)) ||
        /^[+]{1}[3]{1}[8]{1}[7]{1}\d{9}$/.test(String(telefon))
      ) {
        return true;
      } else {
        return false;
              ...
              http
                ...
                .then(res => {
                  if (res.data.success === false) {
                    ...
                    this.toastText = this.$t(
                      "pacijenti.unos.UnosPacijenta.trnslr017"
                    ...
                    setTimeout(() => {
                      this.$router.push(
                        "/pacijenti/pregled?token=" + this.$store.state.token
                      );
                    }, 1000);
                  }
                });
