      ...
      <vuestic-widget :headerText="header">
        <div class="table-responsive">
          <table class="table table-striped first-td-padding">
            <thead>
              <tr>
                <td>{{'pacijenti.pregled.detalji.trnslr001' | translate}}</td>
                <!-- Jmbg -->
                <td>{{'pacijenti.pregled.detalji.trnslr002' | translate}}</td>
                <!-- Ime -->
                <td>{{'pacijenti.pregled.detalji.trnslr003' | translate}}</td>
                <!-- Prezime -->
                <td>{{'pacijenti.pregled.detalji.trnslr004' | translate}}</td>
                <!-- Spol -->
                <td>{{'pacijenti.pregled.detalji.trnslr028' | translate}}</td>
                <td
                  v-if="this.$store.state.site != '5bd40c16020d6d21bbaf610c'"
                >{{'pacijenti.pregled.detalji.trnslr005' | translate}}</td>
                <td
                  v-if="this.$store.state.site != '5bd40c16020d6d21bbaf610c'"
                >{{'pacijenti.pregled.detalji.trnslr006' | translate}}</td>
                <td>{{'pacijenti.pregled.detalji.trnslr007' | translate}}</td>
                <!-- Telefon -->
                <td>{{'pacijenti.pregled.detalji.trnslr008' | translate}}</td>
                <!-- Email -->
                <td>{{'pacijenti.pregled.detalji.trnslr009' | translate}}</td>
                <!-- Uredi -->
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td>
                  ...
                  <input class="table-info" v-show="edit === true" v-model="prezime">
                </td>
                <td>
                  <div v-show="edit === false">
                    <label>
                  ...
                  <select class="table-info" v-show="edit === true" v-model="spol">
                    <option value="MUŠKI">{{'pacijenti.pregled.detalji.trnslr010' | translate}}</option>
                    <!-- MUŠKI -->
                    <option value="ŽENSKI">{{'pacijenti.pregled.detalji.trnslr011' | translate}}</option>
                    <!-- ŽENSKI -->
                  </select>
                </td>
                <td>
                ...
                <td>
                  ...
                  <input class="table-info" v-show="edit === true" v-model="email">
                </td>
                <td>
                  <button
                    v-if="uredi"
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="btn-container">
              <button
                :disabled="edit"
                class="btn btn-sm btn-secondary wizard-back pull-left"
                @click.prevent="vratiPregled"
              >
                <span class="fa fa-step-backward"></span>
                &nbsp{{'pacijenti.pregled.detalji.trnslr016' | translate}}
                <!-- Nazad -->
              </button>
            </div>
          </div>
    ...
    http
      ...
      .then(res => {
        ...
        if (res.data.pacijent.jmbg != undefined) {
          ...
          this.header =
            "Pregled detalja za pacijenta" + " " + res.data.pacijent.jmbg;
        ...
        http
          ...
          .then(res => {
            if (res.data.pacijent.jmbg != undefined) {
              // Pregled detalja za pacijenta:
              this.header =
                this.$t("pacijenti.pregled.detalji.trnslr017") +
                " " +
                res.data.pacijent.jmbg;
              ...
              http
                ...
                .then(res => {
                  if (res.data.success === false) {
                    ...
                    this.toastText = this.$t(
                      "pacijenti.pregled.detalji.trnslr027"
                    ); // Unos uspješno obavljen!
