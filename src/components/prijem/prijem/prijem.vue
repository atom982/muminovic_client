<template>
  <div class="row">
    <div class="col-md-12">
      <vuestic-widget class="no-h-padding" headerText="Prijem pacijenta">
        <vuestic-wizard
          id="wizzard"
          v-if="show"
          v-on:uzorakUnesen="refreshWizard()"
          :steps="hrSteps"
          :sekcija="sekcija"
          wizard-type="simple"
        >
          ...
          <div slot="page1" class="form-wizard-tab-content">
            ...
            <div class="col-md-8">
              <div
                ...
                :class="{'has-error': errors.has('jmbg'), 'valid': isFormFieldValid('jmbg')}"
              >
                <div class="input-group">
                  ...
                  <label class="control-label">{{'Unesite JMBG pacijenta'}}</label>
                  <i class="bar"></i>
                  <small
                    v-show="errors.has('jmbg')"
                    class="help text-danger"
                  >{{ errors.first('jmbg') }}</small>
                </div>
              </div>
              <div
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('ime'), 'valid': isFormFieldValid('ime')}"
              >
                <div class="input-group">
                  <input
                    name="ime"
                  ...
                  <i class="fa fa-exclamation-triangle error-icon icon-right input-icon"></i>
                  <i class="fa fa-check valid-icon icon-right input-icon"></i>
                  <label class="control-label">{{'Unesite ime pacijenta'}}</label>
                  <i class="bar"></i>
                  <small
                    v-show="errors.has('ime')"
                    class="help text-danger"
                  >{{ errors.first('ime') }}</small>
                </div>
              </div>
              <div
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('prezime'), 'valid': isFormFieldValid('prezime')}"
              >
                <div class="input-group">
                  <input
                    name="prezime"
                  ...
                  <i class="fa fa-exclamation-triangle error-icon icon-right input-icon"></i>
                  <i class="fa fa-check valid-icon icon-right input-icon"></i>
                  <label class="control-label">{{'Unesite prezime pacijenta'}}</label>
                  <i class="bar"></i>
                  <small
                    v-show="errors.has('prezime')"
                    class="help text-danger"
                  >{{ errors.first('prezime') }}</small>
                </div>
              </div>
              <vuestic-simple-select
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('spol'), 'valid': isFormFieldValid('spol')}"
              ...
              <vuestic-simple-select
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('lokacija'), 'valid': isFormFieldValid('lokacija')}"
          ...
          <div slot="page2" class="form-wizard-tab-content">
            ...
            <div class="col-md-8">
              <div
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('datum_uzimanja'), 'valid': isFormFieldValid('datum_uzimanja')}"
              >
                <div class="input-group">
                  <input
                    name="datum_uzimanja"
                    data-vv-as="Datum uzimanja"
                    v-model="datum_uzimanja"
                    id="datum_uzimanja"
                    v-validate="'required'"
                    required
                    title=" "
                  ...
                  <i class="fa fa-exclamation-triangle error-icon icon-right input-icon"></i>
                  <i class="fa fa-check valid-icon icon-right input-icon"></i>
                  <label class="control-label">{{"Datum uzimanja"}}</label>
                  <i class="bar"></i>
                  <small
                    v-show="errors.has('datum_uzimanja')"
                    class="help text-danger"
                  >{{ errors.first('datum_uzimanja') }}</small>
                </div>
              </div>
              <div
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('vrijeme_uzimanja'), 'valid': isFormFieldValid('vrijeme_uzimanja')}"
              >
                <div class="input-group">
                  <input
                    name="vrijeme_uzimanja"
                    data-vv-as="Vrijeme uzimanja"
                    v-model="vrijeme_uzimanja"
                    id="vrijeme_uzimanja"
                    v-validate="'required'"
                    required
                    title=" "
                  ...
                  <i class="fa fa-exclamation-triangle error-icon icon-right input-icon"></i>
                  <i class="fa fa-check valid-icon icon-right input-icon"></i>
                  <label class="control-label">{{'Vrijeme uzimanja'}}</label>
                  <i class="bar"></i>
                  <small
                    v-show="errors.has('vrijeme_uzimanja')"
                    class="help text-danger"
                  >{{ errors.first('vrijeme_uzimanja') }}</small>
                </div>
              </div>
              <vuestic-simple-select
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('prioritet'), 'valid': isFormFieldValid('prioritet')}"
                label="Izaberite prioritet"
                v-model="prioritet"
                required
                title=" "
                name="prioritet"
              ...
              ></vuestic-simple-select>
              <vuestic-simple-select
                v-show="$store.state.site != '5bd40c16020d6d21bbaf610c'"
                class="form-group with-icon-right"
                :class="{'has-error': errors.has('doktor'), 'valid': isFormFieldValid('doktor')}"
          ...
          <div slot="page3" class="form-wizard-tab-content">
            ...
            <div class="row container">
              <vuestic-simple-select
                v-if="sveSekcije.length > 2 && !prikaziPanele"
                label="Izaberite sekciju testova"
                v-model="trenutnaSekcija"
                name="sekcija"
                :required="true"
                title=" "
                ref="sekcijaSelect"
                v-bind:options="sveSekcije"
              ></vuestic-simple-select>
              <div v-if="sveSekcije.length < 3 && !prikaziPanele" class="form-group">
                <div class="input-group">
                  <input disabled id="input-icon-left" title=" " required>
                  <label class="control-label" for="sajt">{{sveSekcije[1]}}</label>
                  <i class="bar"></i>
                </div>
              </div>
            </div>
            <div>
              <div
                v-if="this.sviTestovi.length < 1 && !prikaziPanele"
                style="text-align:center;"
                class="table"
              >
              ...
              <div
                v-show="prikaziTestove"
                style="text-align:center;"
                v-for="index in Math.ceil(sviTestovi.length/100) "
                class="table"
              >
                <div
                  v-show="showTest($store.state.site, test.analit, ukGodina, spol, drstanje, menopauza, test.manual, test.calculated)[2] && 
              ...
              <div v-show="prikaziPanele" style="text-align:center;">
                <h4
                  v-if="prikaziPanele && placeholderPaneli != ''"
                  style=" color: #e34a4a"
                >{{placeholderPaneli}}</h4>
                <div
                  v-for="item in paneli"
                  v-bind:id="item.sifra"
                  :title="item.opis"
                  @click="dodajPanel($event, item.sifra)"
                  :class="{'btn btn-secondary btn-micro' : true}"
                  align="middle"
                  :id="item.sifra"
                >{{item.naziv}}</div>
              </div>
              ...
              <div style="text-align:center;">
                <a
                  v-bind:style="{ color: activeColorStyle}"
                  v-if="prikaziTestove"
                  @click="prikPanele"
                  href="#"
                >{{'Paneli laboratorijskih analiza'}}</a>
                <a
                  v-bind:style="{ color: activeColorStyle}"
                  v-if="prikaziPanele"
                  @click="prikTestove"
                  href="#"
                >{{'Laboratorijske analize'}}</a>
                <span>{{'|'}}</span>
                <a
                  v-bind:style="{ color: activeColorStyle}"
                  @click="prikaziDetalje()"
                  href="#"
                >{{'Detalji'}}</a>
              </div>
            </div>
                    ...
                    <div class="col-md-12">
                      <div class="col-md-12">
                        ...
                        <a
                          ...
                          :title="komentar"
                          href="#"
                        >{{'Komentar je unesen'}}</a>
                        <a
                          style="color: #f7cc36"
                          v-if="komentar.trim() === ''"
                          href="#"
                        >{{'Komentar nije unesen'}}</a>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div slot="page4" class="form-wizard-tab-content">
            ...
            <barcode
              ...
              v-if="bar"
              v-bind:value="id"
              :options="{ displayValue: true , width: 1.25, fontSize: 20, textPosition: 'bottom',}"
            ></barcode>
            <button class="btn btn-info" @click="printajBarcode()">
...
import { klasificiraj } from "./klasificiraj_uzorke";
...
export default {
  ...
  mounted() {
    ...
    http
      .get(
        "postavke/doktor/list?token=" +
          this.$store.state.token +
          "&site=" +
          this.$store.state.site,
        {}
      ) // Korak 2 - Izaberite doktora
      .then(res => {
        if (res.data.success === false) {
          this.doktori = [];
          ...
          if (res.data.doktor != 0) {
            if (this.$store.state.site === "5bd40c16020d6d21bbaf610c") {
              ...
              res.data.doktor.forEach(element => {
                this.doktori.push(
                  element.doktorIme + " " + element.doktorPrezime
                );
              });
      ...
      return [
        {
          ...
          isValid: () => {
            return (
              this.isFormFieldValid("ime") &&
              this.isFormFieldValid("prezime") &&
              this.isFormFieldValid("jmbg") &&
              this.isFormFieldValid("lokacija") &&
              this.isFormFieldValid("spol") &&
              this.mojaValidacija1(this.lokacija) &&
              this.mojaValidacija1a(this.lokacija)
            );
          }
        },
        {
          ...
          isValid: () => {
            return (
              this.validateDate(this.datum_uzimanja) &&
              this.validateTime(this.vrijeme_uzimanja) &&
              this.isFormFieldValid("datum_uzimanja") &&
              this.isFormFieldValid("vrijeme_uzimanja") &&
              this.isFormFieldValid("doktor")
            );
          }
        },
        {
          ...
          onNext: () => {
            ...
            var date =
              this.jmbgGodina + "-" + this.jmbgMjesec + "-" + this.jmbgDan;
              ...
              http
                ...
                .then(res => {
                  ...
                  res.data.data.forEach(element => {
                    this.specialTest = klasificiraj(
                      element,
                      this.jmbg,
                      this.ime.toUpperCase().trim(),
                      this.prezime.toUpperCase().trim(),
                      this.spol,
                      this.location,
                      this.datum_uzimanja,
                      this.doctor,
                      this.vrijeme_uzimanja,
                      this.sekcija,
                      this.drstanje,
                      this.menstc,
                      this.anticoag,
                    ...
                    http
                      .post("uzorci/save", {
                        ...
                        prioritet: this.prioritet,
                        komentar: this.komentar,
                        specialTest: this.specialTest
                      })
                      .then(res => {
                        // console.log(res.data.data.specialTest)
                        if (res.data.success) {
                          http
                            .get(
                              "uzorci/barcode/" +
                          ...
                          // console.log('Greška prilikom upisa:'+res.data.message)
                        }

                        if (res.data.data.specialTest.length) {
                          if (res.data.data.specialTest[0].test === "OGTT-60") {
                            //----OGTT-60
                            this.OGTT120 = res.data.data.specialTest[1];
                            // console.log('prije sava ogtt60')
                            // console.log(res.data.data.specialTest[0].json)//60
                            http
                              .post(
                                "uzorci/save",
                                res.data.data.specialTest[0].json
                              )
                              .then(res => {
                                // console.log('Callback od ogtt 60 save')
                                // console.log(this.OGTT120)
                                ...
                                if (res.data.success) {
                                  ...
                                  //-------------------------------------------OGTT-120
                                  if (this.OGTT120.test === "OGTT-120") {
                                    // console.log('prije ogtt 120 save')
                                    // console.log(this.OGTT120)
                                    http
                                      .post("uzorci/save", this.OGTT120.json)
                                      .then(res => {
                                        if (res.data.success) {
                                          http
                                            .get(
                          ...
                          if (
                            res.data.data.specialTest[0].test === "Prolaktin P2"
                          ) {
                            //----Prolaktin pool
                            // console.log("P Pool");
                            // console.log(res.data.data.specialTest);
                            ...
                            http
                              ...
                              .then(res => {
                                if (res.data.success) {
                                  ...
                                  //-------------------------------------------Prolaktin P3
                                  if (this.OGTT120.test === "Prolaktin P3") {
                                    // console.log('prije ogtt 120 save')
                                    // console.log(this.OGTT120)
                                    http
                                      .post("uzorci/save", this.OGTT120.json)
  ...
  watch: {
    ...
    lokacija: function() {
      ...
      http
        .post("/uzorci/sekcije/listp", {
          // Dohvati testove, na promjenu tipa
          token: this.$store.state.token,
          site: this.$store.state.site
        })
        .then(res => {
          ...
          if (res.data.success === false) {
            ...
            res.data.testovi.forEach(element => {
              ...
              if (this.$store.state.site === element.site) {
                if (
                  !this.sveSekcije.filter(test => test === element.sekcija)
                    .length > 0
                ) {
              ...
              if (
                ...
                this.sviTestoviZaSekc.push({
                  // Korak 3 - Testovi u izborniku
                  labassay: element._id,
                  ime_testa: element.naziv,
                  cijena: element.price,
                  analit: element.analit,
  ...
  methods: {
    ...
    mojaValidacija2a(data1, data2, data3) {
      if (
        data1 === "GREŠKA - DB Connection" ||
        data2 === "GREŠKA - DB Connection" ||
        data3 === "GREŠKA - DB Connection"
      ) {
        this.toastText = "Molimo Vas da osvježite aplikaciju!";
        this.toastIcon = "fa-warning";
        this.toastPosition = "top-right";
    ...
    dodajTest(event) {
      ...
      if (
        ...
        // Izvrši samo ako test već nije dodan
    ...
    dodajPanel(event, sifra) {
      this.paneli.forEach(element => {
        if (element.sifra === sifra) {
          ...
          this.ubaceniPanel = [];
          this.izbaceniPanel = [];
          element.testovi.forEach(ptest => {
            // svi testovi unutar panela
            this.tmpPanel.push(ptest);
            this.sviTestovi.forEach(alltest => {
              // filtracija po izabranom tipu
              if (alltest.labassay === ptest.labassay) {
                ...
                if (
                  !this.testovi.filter(test => test.labassay === ptest.labassay)
                    .length > 0
                ) {
                  this.sviTestovi.forEach(element => {
                    if (event.target.id === element.labassay) {
                    ...
                    if (
                      element.calculated &&
                      ptest.labassay === element.labassay
                    ) {
                      calculated = true;
                      element.calculatedTests.forEach(kalk => {
                        if (
                          !this.testovi.filter(
                            test => test.labassay === kalk.labassay
                          ).length > 0
                        ) {
                          kalk.sekcija = element.sekcija;
                          kalk.calculated = false;
                  ...
                  document.getElementById(ptest.labassay).className =
                    "btn btn-info btn-micro";
                } else {
                }
              }
      ...
      this.tmpPanel.forEach(tmp => {
        ...
        if (uslov && i === this.ubaceniPanel.length) {
          if (
            !this.izbaceniPanel.filter(test => test === tmp.ime_testa).length >
            0
          ) {
