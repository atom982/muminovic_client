<template>
  <div class="vuestic-collapse">
    ...
    >
      ...
      >
        <slot name="header"/>
        <span style="font-size:12px">{{' | '}}{{godiste}}</span>
        <div v-if="!isNaN(cijena) && cijena > 0" class="col-md-2 pull-right">
          <span style="text-align:right; padding-top: 0">
            {{cijena}}
            <sup>BAM</sup>
          </span>
        </div>
        <div class="col-md-2 pull-right">
          <span style="text-align:right; padding-top: 0"></span>
        </div>
      </button>
    </div>
...
<script>
export default {
  name: "vuestic-collapse-patient-info",
  props: {
    value: {
      type: Boolean,
      default: false
