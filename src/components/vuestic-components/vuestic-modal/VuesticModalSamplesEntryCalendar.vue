                ...
                <fieldset>
                  <div class="form-group">
                    <div class="input-group">
                      <vuestic-date-picker id="date-picker-time" :config="config" v-model="time"/>
                      <label class="control-label" for="date-picker-range">{{ 'Datum i vrijeme' }}</label>
                    </div>
                  </div>
                </fieldset>
              </div>
...
export default {
  name: "vuestic-modal-samples-entry-calendar",
  props: {
    transition: {
      type: String,
      default: "modal"
  ...
  watch: {
    vrijeme: function(newVal, oldVal) {
      // watch it
  ...
  methods: {
    ...
    ok() {
      this.$emit("ok");
      bus.$emit("setCalendar", this.uzorak, this.time);
      this.show = false
    ...
    open() {
      ...
      // console.log('Inside VuesticModalSamplesEntryCalendar.vue')
    },
    close() {
      this.show = false
      window.removeEventListener("keyup", this.listenKeyUp);
